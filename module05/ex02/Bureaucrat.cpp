...
/*   Updated: 2022/09/19 15:22:52 by arendon-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		Bureaucrat::signForm(Form & formName)
{
	try
	{
		if (this->_grade > formName.getGradeSign())
			throw Form::GradeTooLowException();
		formName.beSigned(*this);
		std::cout << _name << " signed " 
					<< formName.getName() << std::endl;
	}
	catch(const std::exception& e)
	{
		std::cerr << RED << _name << " could't sign "
					<< formName.getName() << " because " << YELLOW << e.what() << RESET << std::endl;
	}
}
...
void		Bureaucrat::executeForm(Form const & form)
{
	...
	{
		std::cerr << RED << _name << " could't execute "
					<< form.getName() << " because " << YELLOW << e.what() << RESET << std::endl;
	}
}
